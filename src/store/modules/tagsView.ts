  ...
  actions: {
    ...
    addVisitedView(view: RouteLocationNormalizedLoaded) {
      if (this.visitedViews.some((v) => v.path === view.path)) {
        const alowMultiTab = view.query?._multiTab || view.query?._multiTab === 'true'
        const existingIndex = this.visitedViews.findIndex((v)=>v.path === view.path)
        if (alowMultiTab) {
          this.handleMultiTab(view)
        }else {
          if (existingIndex > -1) {
            this.visitedViews.splice(existingIndex,1, Object.assign({}, view, 
              { meta: { title: view.query?._tagTitle || view.meta?.title }, title: view.meta?.title || 'no-name' }))
          }
        }
        return
      }
      ...
      this.visitedViews.push(
        Object.assign({}, view, {
          meta: { title: view.query?._tagTitle || view.meta?.title },
          title: view.meta?.title || 'no-name'
        })
      )
    },
    // 多标签处理
    handleMultiTab(view: RouteLocationNormalizedLoaded) {
      ...
      if (visitedView.meta) {
        ...
        this.visitedViews.forEach((v) => {
          if (v.fullPath === visitedView.fullPath && v.meta?.title === visitedView.meta?.title) {
            titleSuffixList.push((v.meta?.titleSuffix || '1') as string)
          }
        })
    ...
    setTitleByFullPath(title: string, fullPath?: string) {
      for (const v of this.visitedViews) {
