...
const columns = [
  { label: '合同流水号', field: 'serialNo', minWidth: 220 },
  { label: '批复流水号', field: 'relativeserialNo', minWidth: 220 },
  { label: '客户编号', field: 'customerID', minWidth: 220 },
  { label: '客户名称', field: 'customerName', minWidth: 320 },
  { label: '业务品种', field: 'businessTypeName', minWidth: 200 },
...
const goDetail = () => {
  ...
  router.push({
    ...
    query: {
      ...
      isEdit,
      creditSourceFlag: '01',
      parentMenuKey: props.params.dealtype?.split('^')[0], // 菜单类型
      t: Date.now(),
      _tagTitle: '项目额度合同详情', // 对tag页签中的title，重命名
    }
  })
}
...
const submit = async (checkFlag) => {
 if(checkFlag != 1){
    const data = await checkCommentExec(currentRow.value.serialNo,"BusinessContract",props.params.dealtype?.split('^')[2])
 ...
 const checkBusType =  await Api.checkBusTypeAndLoanProductCategory({"serialNo":currentRow.value.serialNo,"projectType":"01"})
  ...
  const submitRisk =  await Api.checkSubmitIsRisk({"objectNo":currentRow.value.serialNo,"objectType":"BusinessContract","phaseNo":props.params.dealtype?.split('^')[2]})
    //需要强控
