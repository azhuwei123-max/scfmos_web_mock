...
<!--      </el-form-item>-->
    ...
    <CreditFlowApproval
      ...
      :serial-no="serialNo"
      :phase-no="phaseNo"
      :object-type="'ProjectCoreviewGYL'"
      @confirm="reLoadAll"
    ...
    <approval-opinion ref="approvalOpinionRef" :params="props.params" @submit="submit" />
...
const submit = async () => {
  ...
  const data = await checkCommentExec(serialNo.value,"ProjectCoreviewGYL",phaseNo.value)
  console.log("data:",data)
  if (!data?.phaseOpinion) {
   ElMessage.warning(`${serialNo.value}签署意见未保存`)
    return
  }
