<template>
  <div class="dy-container">
        ...
        <template v-for="item in leftMenuList" :key="item.value">
          <div class="menu-group">
            ...
            >
              ...
              <span class="side-item-text">{{ item.label }}</span>
            ...
            <div v-if="item.expanded && item.children" class="group-children">
              <div
                ...
                :key="childIndex"
                :class="activeMenu === child.value ? 'side-item-select' : 'side-item-default'"
    ...
    <!-- 右侧内容区域（一整块） -->
    <div class="dy-container-right"  v-if="currMenu">
      <ContentWrap>
        <!-- 操作栏 -->
...
<!--          </el-form-item>-->
        ...
        <div   style="display: flex; justify-content: space-between; margin-bottom: 17px">
          <div>
            <el-button @click="handleDetail" plain type="">
              申请详情
            </el-button>
...
// import { defaultQueryParams, tableColumns, statusTypeOptions } from './const'
...
const watchInvesigation =async () => {
  ...
  window.open(res.newreportno,'_blank')
...
const handleExport = async () => {
  try {
    ...
    queryParams.phaseNo = currMenu.value?.phaseNo
    queryParams.flowNo = currMenu.value?.flowNo
    const data = await api.approveExport({...queryParams,type:currentType.value,creditSourceFlag:"01"})
...
//     download.excel(data, '单笔授信项目信息.xls')
//   } catch {
//   } finally {
//     exportLoading.value = false
//   }
