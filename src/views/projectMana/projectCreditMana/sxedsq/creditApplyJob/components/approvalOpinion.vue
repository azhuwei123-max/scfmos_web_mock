<template>
  ...
  <ContentWrap>
    ...
    <!-- Tab 标签页 -->
    <el-tabs v-model="activeTab" :key="activeTab" class="approval-opinion-tabs" >
      <!-- 签署意见 -->
      ...
      <el-tab-pane label="签署客户评级意见" name="rating" lazy>
        ...
        <crmsIframe :params="iframeParams1" />
      </el-tab-pane>

      <!-- 签署贷前资产分类意见 -->
      <el-tab-pane label="签署贷前资产分类意见" name="classification" lazy>
        <!-- <div class="p-20px text-center text-gray-500"> 签署贷前资产分类意见功能待开发 </div> -->
        <crmsIframe :params="iframeParams2" />
      </el-tab-pane>
...
const getApprovalOpinionDetail = async () => {
  try {
    ...
    const ftSerialNo = props.processInstance[0].ftSerialNo
...
const handleSave = async () => {
  ...
  //  })
   ...
   if (!['0010','3000','3010'].includes(props.processInstance[0].phaseNo)) {
    //获取贷前资产风险分类需要的信息
     data = await queryDataAndMakeSaveParam({"objectNo":props.processInstance[0].objectNo,"customerID":props.processInstance[0].customerID})
