...
import { addApproveChangeApply, getApproveChangeApplyList } from '../../api'
...
import apprCustTypePop from '../apprcustypePop/components/apprCustTypePop.vue'
...
const { register, tableObject, tableMethods } = useTable({
  ...
  defaultParams: { dealtype: props.params.dealtype,creditSourceFlag:'02' }
})
  ...
  const result = await addDialog({
    ...
    beforeSure: async (done, { button }) => {
      ...
      if (formData?.serialNo) {
        ...
        await addApproveChangeApply({ serialNo: formData?.serialNo,creditSourceFlag:'02' }).finally(() => {
