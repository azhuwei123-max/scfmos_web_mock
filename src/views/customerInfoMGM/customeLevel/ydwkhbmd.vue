<template>
<!--  <IFrame src="https://crms-dev.ncbank.cn/Credit/ThirdLogon.jsp?TPUSERID=admin&TPPASSWORD=CC357616CC513B4896F8FED824B3E191&TPOPENTYPE=DuebillQueryList&RightType=All" :auto-height='false' />-->
  <ContentWrap>
    <!-- 查询条件 -->
    <div class="mb-15px">
      ...
      >
        ...
        <el-form-item label="流程单/并行" prop="processDefineKey">
          <el-select
            v-model="processDefineKey"
            placeholder="请选择单流程或并行流程"
            clearable
            class="!w-240px"
          >
            <el-option label="单路" value="CustomWhitelist"/>
            <el-option label="并行" value="CustomWhitelistBX"/>
            <el-option label="条件" value="CustomWhitelistTJ"/>
          </el-select>
        </el-form-item>
  ...
  <ContentWrap>
    ...
    >
      ...
      <template #action="{ row }">
        ...
        tpye="" 
        link
        @click="handleSubmit(row.serialNo)" 
...
import SubmitAuditDialog from '@/views/work/audit/SubmitAuditDialog.vue'
...
const processDefineKey = ref('CustomWhitelist') // 流程定义 Key
...
// }
  ...
  // 跳转到详情页面
  await router.push({
    // path: `/customerInfoMGM/customeLevel/ydwkhbmd/detail/${row.serialNo}`,
    name:'cydwkhbmdDetail',
    query: {
...
const handleConfirmSubmit = async (assigness) => {
  console.log('assigness',assigness);
    const data = {serialNo:currentId.value,taskId:'0'}
    data.startUserSelectAssignees = assigness
  data.key = processDefineKey.value
    await AuditWorkApi.submitYdOrder(data)
    await getList()
}
...
const handleBatchComment = () => {
  ...
  if (selectedRows.value.length > 1) {
     message.warning('请仅选择一条填写审批意见')
     return
  }
  // 打开审批意见对话框
 openCommentDialog(selectedRows.value[0].serialNo)
}
...
const openCommentDialog = async (id: number) => {
  ...
  try {
  const data = await AuditWorkApi.getNewComment({
    businessId:id,
    taskId:"0"
  })
  console.log('data',data);
