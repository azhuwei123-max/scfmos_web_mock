<template>
  <ContentWrap v-loading="loading">
    <!-- 操作按钮 - 头部 -->
    <div v-if="!isReadonly" class="flex justify-between items-center mb-20px">
        ...
        <el-col :span="12">
          <el-form-item label="流水号" prop="serialNumber">
            <el-input
              ...
              :disabled="isReadonly || route.params.id !== 'create'"
              placeholder="自动生成"
            />
          </el-form-item>
        </el-col>
...
<!--      </el-row>-->
  ...
  >
    ...
    >
      <el-table-column label="企业编号" prop="customerID" min-width="180" />
      <el-table-column label="企业名称" prop="customerName" min-width="200" show-overflow-tooltip />
      <el-table-column label="营业执照号码" prop="licenseNo" min-width="180" />
      <el-table-column label="法定代表人编号" prop="relativeId" min-width="180" />
      ...
      <el-table-column label="企业基本账户户名" prop="basicBank" min-width="150" />
      <el-table-column label="企业基本账户账号" prop="basicAccount" min-width="150" />
      <!-- <el-table-column label="企业基本账户开户银行" prop="basicBank" min-width="200" show-overflow-tooltip /> -->
    </el-table>
    <!-- 分页 -->
  ...
  >
    <el-table
      ...
      highlight-current-row
    >
      <el-table-column label="银行编号" prop="itemNo" min-width="150" />
      <el-table-column label="银行名称" prop="itemName" min-width="200" show-overflow-tooltip />
...
defineOptions({ name: 'cydwkhbmdDetail' })
...
const isReadonly = computed(()=> !!props.id)
...
// 设置页签标题
const updateTitle = () => {
  const id =  router.currentRoute.value.query.title as string
  ...
  // 编辑模式，使用流水号
  const serialNo = (router.currentRoute.value.query.serialNo as string) || ''
...
const getDetailData = async () => {
  ...
  const serialNo =id || router.currentRoute.value.query.serialNo || props.id || router.currentRoute.value.query.title
  console.log('serialNo',serialNo,router);
...
const handleSave = async () => {
  try {
    ...
    const submitData: any = {
      ...
      basicBankId: formData.basicAccountBankId, // 基础账户企业开户银行 (暂时使用开户银行名称，后续可能需要银行ID)
...
const handleConfirmSelectEnterprise = () => {
  ...
  // 将选中的企业信息填充到表单
  formData.enterpriseId = selectedEnterprise.value.customerID
  formData.enterpriseName = selectedEnterprise.value.customerName
  formData.businessLicenseNumber = selectedEnterprise.value.licenseNo
   formData.legalRepresentativeId = selectedEnterprise.value.relativeId
  ...
  formData.basicAccountNumber = selectedEnterprise.value.basicAccount
  formData.basicAccountName = selectedEnterprise.value.basicBank

  message.success(`已选择企业：${selectedEnterprise.value.customerName}`)
...
const getBankList = async () => {
  try {
    ...
    // 模拟数据 - 生成更多数据用于分页测试
    await new Promise(resolve => setTimeout(resolve, 300))
    const resBankList = await addCustomerApi.getCodeLibraryList({codeNo:"SMTransferBank"})
    console.log(resBankList)
    ...
    const start = (pageNo - 1) * pageSize
    const end = start + pageSize
    const paginatedList = resBankList.slice(start, end)
    
    bankList.value = paginatedList
    bankTotal.value = resBankList.length
  } catch (error) {
...
const handleConfirmSelectBank = () => {
  ...
  // 将选中的银行信息填充到表单
  formData.basicAccountBankId = selectedBank.value.itemNo
  formData.basicAccountBank = selectedBank.value.itemName

  message.success(`已选择银行：${selectedBank.value.itemName}`)
  ...
  try {
    ...
    for (let i = 1; i <= 50; i++) {
      allMockData.push({
        ...
        projectName: `供应链作业平台${String.fromCharCode(64 + ((i - 1) % 26) + 1)}${Math.floor((i - 1) / 26) > 0 ? Math.floor((i - 1) / 26) : ''}`
      })
    }
