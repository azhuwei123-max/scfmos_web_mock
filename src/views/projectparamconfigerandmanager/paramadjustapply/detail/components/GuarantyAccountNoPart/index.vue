...
const templateNoMap: Record<TempType, any> = {
  'ParamAdjustApplyDetail': {
    list: 'GuarantyAccountNoApplyList'
  },
  'ProjectDetails': {
    list: 'GuarantyAccountNoList'
  }
}
...
const add = async (btn: ActionButton) => {
  ...
  const res = await getTemplateInfo({
    templateNo: 'GuarantyAccountNoApplyInfo',
    serialno: props.serialno
  ...
  addDialog({
    ...
    async beforeSure(done, { button }) {
      ...
      if (valid) {
        ...
        await saveCoreBailAdjust(params).finally(() => (button.btn!.loading = false))
  ...
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    async beforeClose(action, instance, done) {
      ...
      const params = {
        ...
        guarantyaccountno: currentRow.guarantyaccountno
      }
      if(props.saveKey) {
        params[props.saveKey] = 1
      }
      ...
      await deleteCoreBailAdjust(params).finally(() => (instance.confirmButtonLoading = false))
