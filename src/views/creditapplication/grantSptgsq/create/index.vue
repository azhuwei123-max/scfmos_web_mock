<template>
  <ContentWrap>
    ...
    >
      ...
      <!-- <el-row v-if="formData.occurrenceType === '084'">
        <el-col :span="12">
          <el-form-item label="续作业务" prop="renewalBusiness" required>
            <el-input
              v-model="formData.renewalBusiness"
      ...
      <el-row>
        ...
        <el-col :span="12">
          <el-form-item v-if="formData.businessTypeNo ==='3030070' || formData.businessTypeNo ==='3030030'" label="合作项目" prop="relaTeamWork">
            <el-input
  ...
  <Dialog v-model="renewalBusinessDialogVisible" title="选择续作业务" width="800px">
    ...
    >
      <el-form-item label="业务编号" prop="businessNo">
        <el-input
          v-model="renewalQueryParams.businessNo"
          class="!w-200px"
          clearable
          placeholder="请输入业务编号"
      ...
      <el-form-item label="业务名称" prop="businessName">
        <el-input
          v-model="renewalQueryParams.businessName"
          class="!w-200px"
          clearable
          placeholder="请输入业务名称"
          @keyup.enter="getRenewalBusinessList"
...
const getRenewalBusinessList =async () => {
  ...
  // 模拟接口调用
  try {
    const res = await penddingApi.qrySequelBusinessPage({
      customerID: formData.value.customerNo,
    })
  console.log('res',res)
    if (res&&res.list) {
...
const handleRenewalBusinessRowClick = (row: any) => {
  formData.value.renewalBusiness = row.businessName
  formData.value.renewalBusinessNo = row.businessNo
...
const formData = ref({
  ...
  //renewalBusinessNo: undefined, // 续作业务编号
