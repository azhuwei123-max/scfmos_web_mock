<template>
   <ContentWrap>
    ...
    <div style="display: flex; margin-bottom: 17px">
      <el-button @click="handleCreate" v-if="(route.query.phaseNo=='0010' || route.query.phaseNo=='3000' || route.query.phaseNo=='1030' ||  checkRoleIds(['289']))  && route.query.type != 'Y'  ">
        新增
      </el-button>
      ...
      <el-button type="" @click="handleDelete" v-if="(route.query.phaseNo=='0010' || route.query.phaseNo=='1030' || route.query.phaseNo=='3000'||  checkRoleIds(['289'])) && route.query.type != 'Y'  ">
        删除
      </el-button>
...
const _pageParams = Object.assign({ objectType: 'CreditApply' }, route.query, props.pageParams)
...
// 删除
  ...
  try {
    //XW2025111800135
      ...
      const res = await associatedCreditApi.getBusinessContract({id:route.query.serialNo})
       if (res?.businesstype && !res?.businesstype.startsWith('3'))  {
          if ( selectedRows.value[0].businesstype=== '3005')  {
            message.warning('业务关联的综合授信不能删除!')
            return;
          }
       
          if(res?.relativeserialno){
             const count = await associatedCreditApi.getOccupyCount({relativeserialno:res?.relativeserialno,id:selectedRows.value[0].relativeserialno})
                if (!(checkRoleIds(['080','480','289']) &&
                    (selectedRows.value[0].businesstype == '3030070' || selectedRows.value[0].businesstype == '3030030')) && count > 0) {
                  message.warning('不可删除授信阶段带入的关联额度!')
                  return;
                }
          }else{
            const res1 = await associatedCreditApi.getRelativeserialno({id:route.query.serialNo})
            const count = await associatedCreditApi.getOccupyCount({relativeserialno:res1,id:selectedRows.value[0].relativeserialno})
             if (!(checkRoleIds(['080','480','289']) &&
                 (selectedRows.value[0].businesstype == '3030070' || selectedRows.value[0].businesstype == '3030030')) && count > 0) {
               message.warning('不可删除授信阶段带入的关联额度!')
               return;
