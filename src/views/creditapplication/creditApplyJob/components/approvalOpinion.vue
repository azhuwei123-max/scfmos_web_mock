<template>
  ...
  <ContentWrap>
    ...
    <!-- Tab 标签页 -->
    <el-tabs v-model="activeTab" :key="activeTab" class="approval-opinion-tabs" @tab-click="tabClick">
      <!-- 签署意见 -->
      <el-tab-pane label="签署意见" name="signing" lazy>
        <el-form
          ref="formRef"
        ...
        >
          ...
          <div class="mb-30px" v-if="formData.showMixin">
            <div class="text-16px font-bold mb-15px flex items-center">
              <Icon icon="ep:arrow-down" class="mr-5px" />
              混合供应链限额管理
            </div>
            <el-row>
              <el-col :span="12">
               <el-form-item label="供应链额度金额" prop="spcsBusinessSum" required>
               <el-input
                v-model="formData.spcsBusinessSum"
                placeholder="供应链额度金额"
                type="number"
                />
              </el-form-item>
            ...
            <el-col :span="12">
               <el-form-item label="供应链敞口金额" prop="spcsExposureSum" required>
               <el-input
                v-model="formData.spcsExposureSum"
                placeholder="供应链敞口金额"
                type="number"
                />
              </el-form-item>
            ...
            <el-row>
              <el-col :span="12">
               <el-form-item label="非供应链额度金额" prop="nspcsBusinessSum" required>
               <el-input
                v-model="formData.nspcsBusinessSum"
                placeholder="非供应链额度金额"
                type="number"
            ...
            <el-col :span="12">
               <el-form-item label="非供应链敞口金额" prop="nspcsExposureSum" required>
               <el-input
                v-model="formData.nspcsExposureSum"
                placeholder="非供应链敞口金额"
                type="number"
                />
              </el-form-item>
            </el-col>
      ...
      <!-- 签署客户评级意见 -->
      <el-tab-pane label="签署客户评级意见" name="rating" lazy>
        <!-- <div class="p-20px text-center text-gray-500"> 签署客户评级意见功能待开发 </div> -->
        <crmsIframe v-if="iframeParams1" :params="iframeParams1"  />
      </el-tab-pane>

      <!-- 签署贷前资产分类意见 -->
      <el-tab-pane label="签署贷前资产分类意见" name="classification" v-if="props.processInstance[0].businesstype != '3030070'" lazy>
        <!-- <div class="p-20px text-center text-gray-500"> 签署贷前资产分类意见功能待开发 </div> -->
        <crmsIframe v-if="iframeParams2" :params="iframeParams2"  />
      </el-tab-pane>

      <!-- 本行信贷业务历史申请记录查询 -->
      <el-tab-pane label="本行信贷业务历史申请记录查询" name="history" lazy>
        <!-- <div class="p-20px text-center text-gray-500"> 本行信贷业务历史申请记录查询功能待开发 </div> -->
        <crmsIframe :params="iframeParams3" />
      </el-tab-pane>
...
import useCredit from '@/hooks/web/business/credit/useCredit.js'
import { relative } from 'path/posix'

const { queryDataAndMakeSaveParam } = useCredit()
...
const formRules = reactive({
  ...
  spcsBusinessSum: [
    { required: true, message: '供应链额度金额不能为空', trigger: 'blur' }
  ],
  spcsExposureSum: [
    { required: true, message: '供应链敞口金额不能为空', trigger: 'blur' }
  ],
  nspcsBusinessSum: [
    { required: true, message: '非供应链额度金额不能为空', trigger: 'blur' }
  ],
  nspcsExposureSum: [
    { required: true, message: '非供应链敞口金额不能为空', trigger: 'blur' }
  ],
  termMonth:[
  ...
  exposureSum:[
  { required: true, message: '授信敞口不能为空', trigger: 'blur' }
  ],
})
...
const getApprovalOpinionDetail = async () => {
  try {
    ...
    const res = await getApprovalOpinionDetailApi({
      businessId,
      objectType:'CreditApply',
      ftSerialNo:props.processInstance[0].ftSerialNo
    })
    ...
    if (res) {
      ...
      formData.ftSerialNo = res.ftSerialNo
    } else {
      // 初始化操作信息（从用户信息获取）
...
const handleSave = async () => {
  ...
  //  })
   ...
   //非 0010 3000 3010阶段展示
   let data ={}
   if (!['0010','3000','3010'].includes(props.processInstance[0].phaseNo)) {
    //获取贷前资产风险分类需要的信息
     data = await queryDataAndMakeSaveParam({"objectNo":props.processInstance[0].objectNo,"customerID":props.processInstance[0].customerid || props.processInstance[0].customerID})
    ...
    await saveApprovalOpinion(Object.assign(formData,data));
...
const handleSubmit = async () => {
  ...
  try {
    ...
    await ElMessageBox.confirm('确定要提交吗？提交后将无法修改。', '提示', {
...
const handleDelete = async () => {
  try {
    ...
    // 调用 API 删除审批意见
    // const applicationId = Array.isArray(route.params.id) ? route.params.id[0] : route.params.id
    await removeApprovalOpinion({id:resData.value.businessId,ftSerialNo:formData.ftSerialNo})
