<template>
  <ContentWrap>
    ...
    <el-dialog
      ...
      destroy-on-close
    >
      <creditApplyJobCreate       :creditSourceFlag = "props.creditSourceFlag"  @close="handleClose" @handle-success="handleSuccess" />
...
import { checkViewComments } from '@/api/common'
...
const checkWhiteApply = (row) => {
  ...
  return new Promise((resolve, rej) => {
    applicationPenddingApi
      .checkWhiteApply({ objectNo: row.objectNo,noCheck:true })
      .then((res) => {
        if (!res) {
          resolve(false)
          return
...
const handleDetail = async () => {
  ...
  router.push({
    ...
    query: {
      ...
      _tagTitle: '链属授信详情', // 对tag页签中的title，重命名
    }
  })
...
//     detail:()=> !hasSin
// }
const { roleIds } = useUserStoreWithOut()
const roles = ref([])
async function getCheckViewComments() {
  const res = await checkViewComments()
  roles.value = res
}
getCheckViewComments()
...
const viewPhaseOpinion = () => {
  ...
  if(currentPhaseType.value === '1020') {
    const isRole = roles.value.some(s => roleIds.includes(s))
    if(!isRole) {
      message.warning('当前业务未完成审批，暂时不允许查看意见，如有疑问请联系总行信贷审批部！')
      return 
    }
  }
  console.log('viewPhaseOpinionRef.value',viewPhaseOpinionRef.value);
...
const handleExport = async () => {
  try {
    ...
    const data = await creditApplyJobApi.export({ ...queryParams,creditSourceFlag:props.creditSourceFlag ,phaseType:currentPhaseType.value})
    download.excel(data, '授信额度申请列表.xls')
  } catch {
  } finally {
...
//   loading2.value = true
//
//   creditApplyJobApi.export(queryParams).then(res => {
