      ...
      <el-row>
        <el-col :span="12">
          <el-form-item v-if="formData.id === undefined" label="用户名称" prop="username">
            <el-input v-model="formData.username" autocomplete="new-password" placeholder="请输入用户名称" />
          </el-form-item>
        </el-col>
        <el-col :span="12">
...
const equalToPassword = (_rule, value, callback) => {
  if (!strongRegex.test(formData.value.password)) {
...
const formRules = reactive<FormRules>({
  ...
  deptId:[{ required: true, message: '部门归属不能为空', trigger: 'change'}],
  email: [
    {
      type: 'email',
      message: '请输入正确的邮箱地址',
