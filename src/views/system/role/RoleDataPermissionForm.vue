<template>
  <Dialog v-model="dialogVisible" title="数据权限" width="800">
    ...
    <el-form-item
      ...
      style="display: flex"
    >
      <el-card class="card" shadow="never" style="width:900px">
        <template #header>
          ...
          />
          <span  style="margin-left: 44px;margin-right: 8px"> 全部展开</span>
          <el-switch
            v-model="deptExpand"
            inline-prompt
            @change="handleCheckedTreeExpand"
          />
          <span  style="margin-left: 44px;margin-right: 8px">父子联动</span>
          <el-switch v-model="checkStrictly"  inline-prompt />
        </template>
        <el-tree
          ref="treeRef"
