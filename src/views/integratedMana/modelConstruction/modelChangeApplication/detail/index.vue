...
import useFormChange from './hooks/useFormChange.js'
...
const templateNo = 'ModelChangeApplyInfo'
...
async function handleTempData() {
  try {
    ...
    dockList.push(
      ...(temp?.docks || []).map((item) => {
        if (item.dockid === 'ModelApplyInfoRelativePart' && tempValue.modeladjusttype === '01') {
          item.isHidden = true
         }
        return {
          ...item,
...
const removeFile = async (index) => {
  ...
  if(formData.uploadfile) {
    await ElMessageBox.confirm('是否删除改附件', '提示', {
...
const handleSelectChange = (formInfo) => {
  dynamicFormRef.value.hiddenDock(
      'ModelApplyInfoRelativePart',
      formData?.modeladjusttype === "01"
    )
  formSelectChange(formInfo)
}
