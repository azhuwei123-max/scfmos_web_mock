<template>
  <ContentWrap v-loading="pageLoading">
    ...
    <div style="display: flex; margin-bottom: 17px">
      ...
      <el-button  v-if="viewPhaseOpinionVif" @click="handleSignOpinion">
        签署意见
      </el-button>
      ...
      <el-button  v-if="signOpinionVIf" @click="viewPhaseOpinion">
        查看意见
      </el-button>
      <el-button  v-if="viewRecordVIf" @click="toRecord">
        查看流转记录
      </el-button>
...
// 签署意见 
const viewPhaseOpinionVif = computed(() => ['010','040'].includes(props.params.key))

// 提交
const submitVIF =  computed(() => ['010','040'].includes(props.params.key))

//  查看意见 
const signOpinionVIf = computed(() => ['020','030','040','050'].includes(props.params.key))

// 查看流转记录
const viewRecordVIf = computed(() => ['020','030','050'].includes(props.params.key))

// 收回
const takeBackVIf = computed(() => {
  return ['020'].includes(props.params.key)
})
...
const {
  ...
  tableCurrentChange,
  fetchCback
} = useTableList(Api.getProjectCoreviewPage, { phaseType: props.params.dealtype, modelApplyType: '02' }, false)
...
fetchCback((list) => {
  list.forEach((v) => {
    ...
    v.modeladjusttype1 = getDictLabels('ModelAdjustType', v.modeladjusttype)
    if (v.modelcategory) {
...
const initFetch = () => {
  Promise.all([getDictFetch(['GYLLoanProductCategory', 'ModelAdjustType'])]).then(res => {
    getList()
  })
}
...
const columns = [
  ...
  { label: '变更类型', field: 'modeladjusttype1', minWidth: 130 },
...
const goDetail = () => {
  ...
  router.push({
    ...
    query: {
      ...
      type: ['010','040'].includes(props.params.key)?'edit':'readonly',
      readonly: ['010','040'].includes(props.params.key)?'0':'1',
 ...
 partenterPartFormTempList.value =  res.templates.map((item) => ({
  ...
  addDialog({
    ...
    contentRenderer: ({options,index}) => (
      <div>
        <div>
          <el-button onClick={()=> addConfirm('2',options, index)}>保存</el-button>
          <el-button onClick={()=> addConfirm('1')}>暂存</el-button>
        </div>
        <simpleForm
          ref={(el) => {
...
const handleSignOpinion = () => {
  ...
  currentProcessInstanceForOpinion.value = {
    ...
    phaseType:props.params.dealtype,
...
const submit = async () => {
  ...
  if (currentRow.value['tempsaveflag'] != 2) {
     return ElMessage.warning('请保存信息后再提交')
  }

   const data = await checkCommentExec(currentRow.value.serialno,"GYLModelApply",props.params.dealtype)
  ...
  phaseType.value =  props.params.dealtype
...
console.log('phaseNo: props.params.phaseNo,',props.params,props.params.phaseNo);
