      ...
      <div class="flex flex-col items-start gap2" :id="`activity-task-${activity.id}-${index}`">
        ...
        >
          ...
          <el-tooltip content="添加用户" placement="left">
            <el-button
              class="!px-6px"
              @click="handleSelectUser(activity.id, customApproveUsers[activity.id],index)"
            >
...
import { ActivityNodes, SimpleModel } from './type'
...
const handleSelectUser = (activityId, selectedList,index) => {
...
const handleUserSelectConfirm = (activityId: string, userList: any[]) => {
  ...
  console.log('customApproveUsers.value[activityId]',activityId,customApproveUsers.value[activityId],userList);
