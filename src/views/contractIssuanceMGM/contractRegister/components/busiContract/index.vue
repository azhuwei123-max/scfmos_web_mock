...
const {
  ...
  fetchCback,
  setCurrentRow
} = useTableList(Api.getYWCreditBusContraPage, { dealType: props.params.dealtype,creditSourceFlag:'02' }, false)
...
// 这个页面被多个页面嵌套，所以统一使用1010这种格式判断，否则很多页面是类似040010，太个性化
const menuKey = props.params.dealtype.split('^')[0]

// 取消合同显隐
const cancelContractVIf = computed(() => {
  return ['1010','1025','1030'].includes(menuKey)
})
...
const addBusiTypeVIf = computed(() => {
  return ['1010','1025'].includes(menuKey)
})
...
const viewOpinionIf = computed(() => {
  return !['1010','1070'].includes(menuKey)
})

// 集中作业任务挑选
const concentratedWorkOptionVIf = computed(() => {
  return ['1070'].includes(menuKey)
})
// 集中作业任务退回
const concentratedWorkBackVIf = computed(() => {
  return ['1010'].includes(menuKey)
})
...
const goDetail = () => {
  ...
  const has289 = checkRoleIds(['289'])
   const isEdit = (['0010','3000'].includes(currentRow.value.phaseno)) || (props.params.key ==='040020' && has289)
  ...
  router.push({
    ...
    query: {
      ...
      readonly: props.params.dealtype?.split('^')[2] || ['040020'].includes(props.params.key),
      customerType:  currentRow.value.customerType,
      isEdit,
      creditSourceFlag: '02',
      parentMenuKey: props.params.dealtype?.split('^')[0],
      t: Date.now(),
      _tagTitle: '链属业务合同详情', // 对tag页签中的title，重命名
    }
  })
}
...
const addBusiType = () => {
  addBusiTypePopRef.value.open({creditSourceFlag:'02',parentMenuKey:menuKey})
}
...
const concentratedWorkOption = () => {
  ...
  confirmFetch({
    title: '是否确定集中作业挑选？',
    fetchFunc: () =>
      Api.concentratedWorkOption({ serialNo: currentRow.value.serialNo, objectType: 'BusinessContract',CreditSourceFlag:props.params?.creditSourceFlag ?? '02' })
  }).then((_) => {
...
const concentratedWorkBack = () => {
  ...
  confirmFetch({
    title: '是否确定集中作业退回？',
    fetchFunc: () =>
      Api.concentratedWorkBack({ serialNo: currentRow.value.serialNo, objectType: 'BusinessContract',CreditSourceFlag:props.params?.creditSourceFlag ?? '02' })
  }).then((_) => {
