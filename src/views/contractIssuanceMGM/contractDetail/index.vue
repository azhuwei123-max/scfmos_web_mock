...
const tabs = ref([
  ...
  },
  {
    ...
    label: '批复详情',
    show: ()=> !!baseData.value?.bapSerialNo,
    onClick: (btn) => handleTabsChange(btn)
  },
  {
    key: 'creditApplyJobDetail',
    label: '申请详情',
    show: ()=> !!baseData.value?.baSerialNo,
    onClick: (btn) => handleTabsChange(btn)
  },
  {
    key: 'cusDetail',
...
const toImageInfo = async (btn) => {
  try {
    ...
    const params = {
      ...
      objectType: 'BusinessContract',
      customerID: route.query.customerID || route.query.customerId || route.query.customerid || baseData.value.customerId
    }
