<template>
  <Dialog v-model="visible" title="额度分配信息详情" width="1000px" top="5vh" v-loading="loading">
    <el-scrollbar>
      <simpleForm2
        ref="dynamicFormRef"
...
const open = (row,isEdit,serialNo) => {
  ...
  if (formTempList.value.length) {
    loading.value = true
    qryQuotaAllocationInfoItem(row).finally(() => (loading.value = false))
  } else {
    initFetch(row)
  }
}
...
const qryQuotaAllocationInfoItem = (row) => {
  Object.keys(formData).forEach((key) => delete formData[key])
  return Api.qryQuotaAllocationInfoItem({ serialNo: row.serialno }).then((res) => {
...
const initFetch = (row) => {
  loading.value = true
  Promise.all([qryQuotaAllocationInfoItem(row), getTemplateVO('CreditAllotInfo')]).finally(
    () => (loading.value = false)
  )
}
