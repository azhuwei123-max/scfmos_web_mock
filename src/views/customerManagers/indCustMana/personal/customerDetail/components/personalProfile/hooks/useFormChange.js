...
export default function useFormChange(props, refList) {
  ...
  const formSelectChange = (formInfo) => {
    ...
    switch (formInfo['colname']) {
      case 'workbegindate': // 本单位工作起始日
        if (!val) {
          props.formData['worktime'] = 0
          return
        }

        const dt1 = dayjs(val)
        const dfMon = dayjs().diff(dt1, 'month', true)
        props.formData['worktime'] = Math.ceil(dfMon / 12)
        break
      case 'ifliveout':
  ...
  const popConfirm = (colname, item) => {
    switch (colname) {
      ...
      case 'nativeplace': // 注册省市区
        ...
        props.formData.regionalism = item.id
        break
      case 'workadd': // 注册省市区
      ...
      case 'unitkindname': // 国标行业分类
        ...
        break
      case 'entorgcustomername': // 主经营
        props.formData.entorgcustomerid = item.customerid
        props.formData.entorgcustomername = item.customername
        break
      default:
        break
    }
   ...
   */
  const customVisible = {
    profstatus :() => props.formData['occupation'] == 'Y',
    ...
    worktime: () => !['27', '31', 'Y', 'X', 'Z'].includes(props.formData['occupation'])
  }
  /**
   * 自定义字段必录逻辑
   */
  const customRequired = {
    ...
    //headshipdetail: () => !['27', '31','Y','X','Z'].includes(props.formData['occupation']),
    headshiptime: () => !['27', '31','Y','X','Z'].includes(props.formData['occupation']),
    unitkindname: () => !['27', '31', 'Y', 'X', 'Z'].includes(props.formData['occupation']),
    workcorp:() => !['27', '31', 'Y', 'X', 'Z'].includes(props.formData['occupation']),
    workadd:() => !['27', '31', 'Y', 'X', 'Z'].includes(props.formData['occupation']),

    nativeplace: () => props.formData['countryname'] === '中国',
    entorgcustomername: ()=> ['010','020'].includes(props.formData['bosstype'])
  }
  /**
   * 自定义字段只读逻辑
   */
