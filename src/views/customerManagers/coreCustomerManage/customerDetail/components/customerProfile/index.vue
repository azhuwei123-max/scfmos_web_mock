...
const getCustomerTemplateNo = () => {
  return Api.getCustomerTemplateNo({ codeNo: 'CustomerOrgType', itemNo: route.query.orgNature })
}
...
const initGetData = () => {
  if (!route.query.orgNature) {
    return ElMessage.error('无机构类型333')
  }

  loading.value = true
  Promise.all([getCustomerTemplateNo(), getDetail()]).then((res) => {
    getTemplateVO(res[0], formData.customertype)
  })
}
