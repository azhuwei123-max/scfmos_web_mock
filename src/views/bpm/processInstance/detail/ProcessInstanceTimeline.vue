...
<template>
      ...
      <div class="flex flex-col items-start gap2" :id="`activity-task-${activity.id}-${index}`">
        ...
        <div v-else class="flex items-center flex-wrap mt-1 gap2">
          ...
          <div v-for="(task, idx) in activity.tasks" :key="idx" class="flex flex-col pr-2 gap2">
            ...
            >
              ...
              >
                <template v-if="task.assigneeUser?.avatar || task.assigneeUser?.nickname">
                  ...
                  {{ task.assigneeUser?.nickname }} ({{ task.assigneeUser?.username }})
                </template>
                <template v-else-if="task.ownerUser?.avatar || task.ownerUser?.nickname">
                  ...
                  {{ task.ownerUser?.nickname }} ({{ task.assigneeUser?.username }})
                </template>
                <!-- 信息：任务 ICON -->
                <div
            ...
            <div defer :to="`#activity-task-${activity.id}-${index}`">
              <div
                v-if="
                  task.reason &&
  ...
  <!-- 用户选择弹窗 -->
  <UserSelectFormNew ref="userSelectFormRef" @confirm="handleUserSelectConfirm" />
</template>
...
import UserSelectFormNew from '@/components/UserSelectForm/index2.vue'
import { ActivityNodes, SimpleModel } from '@/views/work/audit/type'
...
// 处理所有activity 节点为一维数组
const extractActivityNodes = (rootNode: SimpleModel): ActivityNodes[]  => {
  const result: ActivityNodes[] = []
  const stack: SimpleModel[] = [rootNode]

  while (stack.length > 0) {
    const currentNode = stack.pop()

    if (currentNode?.type === NodeType.USER_TASK_NODE) {
      result.push({
        id: currentNode.id,
        bpmSupplyconditionVO: currentNode.bpmSupplyconditionVO,
    ...
    if(currentNode?.conditionNodes && Array.isArray(currentNode.conditionNodes)) {
      for (const condition of currentNode.conditionNodes) {
...
const supplyconditionVOList = computed(()=> {
  return extractActivityNodes((props.simpleModel || {}) as SimpleModel)
})

// const findindWithBpmSupplyconditionVO = (node,result=[]) => {
//    if(!node) return result

//    if(node.bpmSupplyconditionVO) {
//      result.push(node.bpmSupplyconditionVO)
//    }

//    if(node.childNode && typeof node.childNode === 'object') {
//     findNodeWithBpmSupplyconditionVO(node.childNode,result)
//    }
//    return result
// }
...
const handleSelectUser = (activityId, selectedList) => {
  ...
  console.log('[ supplyconditionVOList ] >', supplyconditionVOList.value)
  const params = supplyconditionVOList.value.find(item=> item?.id === activityId)?.bpmSupplyconditionVO
  console.log('[ params  ] >', params)
  
  userSelectFormRef.value.open(activityId, selectedList, params)
}
