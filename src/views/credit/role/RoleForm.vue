<template>
  <Dialog :title="dialogTitle" v-model="dialogVisible">
    ...
    >
      ...
      <el-form-item label="角色权限字符串" prop="code">
        <el-input v-model="formData.code" placeholder="请输入角色权限字符串" />
      </el-form-item>
...
const formRules = reactive({
  ...
  code: [{ required: true, message: '角色权限字符串不能为空', trigger: 'blur' }],
  sort: [{ required: true, message: '显示顺序不能为空', trigger: 'blur' }],
  // status: [{ required: true, message: '角色状态（0正常 1停用）不能为空', trigger: 'blur' }],
  type: [{ required: true, message: '角色类型不能为空', trigger: 'change' }]
})
