...
export type EventType = 'open' | 'close' | 'openAutoFocus' | 'closeAutoFocus' | 'fullscreenCallBack'

export type ArgsType = {
  /** `cancel` 点击取消按钮、`sure` 点击确定按钮、`close` 点击右上角关闭按钮或空白页或按下了esc键 */
  command: 'cancel' | 'sure' | 'close'
  /** 附加数据，可由 contentRenderer 组件通过 @close 事件传递 */
  data?: any
}

/** addDialog 的 Promise 返回类型 */
...
export type DialogProps = {
  ...
  /** `Dialog` 的宽度，默认 `50%` */
  width?: string | number
  /** 是否为全屏 `Dialog`（会一直处于全屏状态，除非弹框关闭），默认 `false` */
  fullscreen?: boolean
  /** 是否显示全屏操作图标，默认 `false` */
  ...
  /** 是否需要遮罩层，默认 `true` */
  modal?: boolean
  /** `Dialog` 自身是否插入至 `body` 元素上，默认 `false` */
...
export type Popconfirm = {
  ...
  /** 关闭时的延迟，默认 `200` */
  hideAfter?: number
  /** 是否将 `popover` 插入至 `body` 元素，默认 `true` */
  teleported?: boolean
  /** 当 `popover` 组件长时间不触发且 `persistent` 属性设置为 `false` 时会被删除，默认 `false` */
  persistent?: boolean
...
export interface DialogOptions extends DialogProps {
  /** @internal 对话框唯一标识符（内部使用，自动生成） */
  _id?: string
  /** @internal Promise resolve 函数（内部使用） */
  _resolve?: (result: DialogResult) => void
  /** @internal Promise reject 函数（内部使用） */
  ...
  /** 自定义按钮操作区的内容渲染器，会覆盖 `footerButtons` 以及默认的取消和确定按钮 */
  footerRenderer?: ({
    options,
    index
  }: {
