<template>
  <ElDialog v-if="isModal" v-model="showSearch" :show-close="false" title="菜单搜索" :close-on-click-modal="false">
    <el-select
      filterable
  ...
  <div v-else class="custom-hover top-search" @click.stop="showTopSearch = !showTopSearch">
    <Icon icon="ep:search" class="top-search__icon" />
    <el-select
      filterable
      :reserve-keyword="false"
      remote
...
.top-search {
  ...
  &__icon {
    font-size: 20px;
    z-index: 1000;
    color: var(--top-header-text-color, var(--el-text-color-regular));
