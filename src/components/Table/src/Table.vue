...
export default defineComponent({
  ...
  props: {
    ...
    size: propTypes.string.validate((v: string) => ['','large', 'default', 'small'].includes(v)).def('small')
  },
  emits: ['update:pageSize', 'update:currentPage', 'register', 'selection-change', 'page-change'],
  setup(props, { attrs, slots, emit, expose }) {
    ...
    const currentChange = (pageNo: Number) => {
      emit('page-change', { pageNo, pageSize: pageSizeRef.value })
    }
    
    const sizeChange = (pageSize: Number) => {
      emit('page-change', { pageNo: currentPageRef.value, pageSize })
    }
