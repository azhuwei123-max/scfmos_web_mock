    ...
    <div v-for="(listener, listenerIdx) in taskListener" :key="listenerIdx">
      ...
      <el-form-item>
        <el-switch
          ...
          inactive-text="关闭"
        />
        <el-select v-if="configForm[`task${listener.type}ListenerEnable`]" v-model="configForm[`task${listener.type}ListenerType`]" placeholder="请选择成员">
          <el-option label="JavaBean" value="1"/>
          <el-option label="Feign请求" value="2"/>
        </el-select>
      </el-form-item>
      <div v-if="configForm[`task${listener.type}ListenerEnable`] && configForm[`task${listener.type}ListenerType`] === '1'">
        <el-form-item
          label="JavaBeanName"
          :prop="`task${listener.type}ListenerJavaBeanName`"
          :rules="{
            required: true,
            message: 'JavaBeanName不能为空',
            trigger: 'blur'
          }"
        >
          <el-input v-model="configForm[`task${listener.type}ListenerJavaBeanName`]" />
        </el-form-item>
      </div>
      <div v-if="configForm[`task${listener.type}ListenerEnable`] && configForm[`task${listener.type}ListenerType`] === '2'">
        <el-form-item>
          <el-alert
