<template>
  ...
  >
        ...
        <div>
          <el-form ref="formRef" :model="configForm" label-position="top" :rules="formRules">
            <el-form-item :label="`${nodeTypeName}人设置`" prop="candidateStrategy">
              ...
              >
                <el-row>
                  <el-col v-for="(dict, index) in CANDIDATE_STRATEGY" :key="index" :span="24">
            ...
            <el-form-item
              v-if="
              configForm.candidateStrategy == CandidateStrategy.START_USER_SELECT   
              "
              label="指定角色"
              prop="applyRole"
            >
              <el-select
                filterable
                v-model="configForm.bpmSupplyconditionVO.applyRole"
                clearable
                multiple
                style="width: 100%"
              >
            ...
            <el-form-item
              v-if="
              configForm.candidateStrategy === CandidateStrategy.APPROVE_USER_SELECT
              "
              label="指定角色"
              prop="approveRole"
            >
              <el-select
                filterable
                v-model="configForm.bpmSupplyconditionVO.approveRole"
                clearable
                multiple
                style="width: 100%"
              >
            ...
            v-if="
              ...
              "
              label="发起人同机构"
              prop="bpmSupplyconditionVO"
            >
            <el-radio-group v-model="configForm.bpmSupplyconditionVO.sponDepFlag">
            <el-radio    
              :key="YES"
              :value="true"
              label="是"
      ...
      >
        <div class="button-setting-pane">
          ...
          <div class="button-setting-item" 
            ...
            :key="index"
            v-show="isButtonVisible(item.id)"
          >
    ...
    <template #footer>
      <el-divider />
      <div>
        <el-button tpye="" @click="saveConfig">确 定</el-button>
...
const formRules = reactive({
  ...
  applyRole: [{
    validator:(rule:any,value: any,callback:any)=> {
       ...
       if(configForm.value.bpmSupplyconditionVO.applyRole.length ==0) {
  ...
  approveRole: [{ 
    validator:(rule:any,value: any,callback:any)=> {
       if(configForm.value.bpmSupplyconditionVO.sponDepFlag.length ==0) {
         callback(new Error('角色不能为空'))
         return
       }
       callback()
  ...
  bpmSupplyconditionVO:[{
    validator:(rule:any,value: any,callback:any)=> {
    
       if(value.sponDepFlag=='') {
         callback(new Error('请选择发起人同机构'))
         return
       }
       callback()
    },
...
const changeCandidateStrategy = () => {
  ...
  configForm.value.applyRole = []
  configForm.value.approveRole = []
...
/** 判断按钮是否在UI中显示 */
const isButtonVisible = (buttonId: OperationButtonType) => {
  const hiddenButtons = [
    OperationButtonType.TRANSFER, // 5 转办
    OperationButtonType.DELEGATE, // 5 委派
    OperationButtonType.ADD_SIGN, // 5 加签
    OperationButtonType.REJECT,  // 6 退回
  ]
...
const saveConfig = async () => {
  ...
  currentNode.value.taskCreateListener = {
    ...
    type: configForm.value.taskCreateListenerType,
    javaBeanName: configForm.value.taskCreateListenerJavaBeanName,
  ...
  currentNode.value.taskAssignListener = {
    ...
    type: configForm.value.taskAssignListenerType,
    javaBeanName: configForm.value.taskAssignListenerJavaBeanName,
  ...
  currentNode.value.taskCompleteListener = {
    ...
    type: configForm.value.taskCompleteListenerType,
    javaBeanName: configForm.value.taskCompleteListenerJavaBeanName,
  ...
  // 发起人同机构
  // currentNode.value.sponDepFlag = configForm.value.sponDepFlag

  // // 指定角色
  // currentNode.value.applyRole = configForm.value.applyRole
  // currentNode.value.approveRole = configForm.value.approveRole


  currentNode.value.bpmSupplyconditionVO = configForm.value.bpmSupplyconditionVO
...
const showUserTaskNodeConfig = (node: SimpleFlowNode) => {
  ...
  configForm.value.taskCreateListenerType = node.taskCreateListener?.type
  configForm.value.taskCreateListenerJavaBeanName = node.taskCreateListener?.javaBeanName
  ...
  configForm.value.taskAssignListenerType = node.taskAssignListener?.type
  configForm.value.taskAssignListenerJavaBeanName = node.taskAssignListener?.javaBeanName
  ...
  configForm.value.taskCompleteListenerType = node.taskCompleteListener?.type
  configForm.value.taskCompleteListenerJavaBeanName = node.taskCompleteListener?.javaBeanName
  ...
  configForm.value.bpmSupplyconditionVO ={ 
    sponDepFlag: node?.bpmSupplyconditionVO?.sponDepFlag ?? '',
    applyRole:node?.bpmSupplyconditionVO?.applyRole ?? [],
    approveRole:node?.bpmSupplyconditionVO?.approveRole ?? []
  }


  // configForm.value.sponDepFlag = node?.sponDepFlag ?? ''

  // configForm.value.applyRole = node?.applyRole ?? []
  // configForm.value.approveRole = node?.approveRole ?? []
}
